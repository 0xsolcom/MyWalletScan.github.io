...
const renzo = async (address) => {
    ...
    const url = `https://airdroptool.vip/api/renzo.php?address=${address}`;
    ...
    try {
        ...
        if (data && typeof data === 'object') {
            const renzoPoints = (Number(data.renzoPoints)).toFixed(2);
            //const eigenLayerPoints = parseInt(Number(data.eigenLayerPoints), 10);
            const eigenLayerPoints = (Number(data.eigenLayerPoints)).toFixed(2);
