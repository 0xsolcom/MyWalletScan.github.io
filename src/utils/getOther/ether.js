...
const ether = async (address) => {
    ...
    const url = `https://app.ether.fi/api/portfolio/v3/${address}`;
    ...
    try {
        ...
        if (data && typeof data === 'object') {
            const totalIntegrationEigenLayerPoints = Number(data.totalIntegrationEigenLayerPoints);
            const totalIntegrationLoyaltyPoints = Number(data.totalIntegrationLoyaltyPoints);
