...
const ether = async (address) => {
    ...
    try {
        ...
        if (data && typeof data === 'object') {
            const totalIntegrationEigenLayerPoints = parseInt(Number(data.totalIntegrationEigenLayerPoints)).toFixed(0);
            const totalIntegrationLoyaltyPoints = parseInt(Number(data.totalIntegrationLoyaltyPoints)).toFixed(0);
