...
import getBaseData from "@utils/getBase/main.js";
import './index.css'
...
const Base = () => {
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (text, record) => {
                if (hideColumn) {
                    return <a href={`https://debank.com/profile/${record.address}?chain=base`} target="_blank">{text.slice(0, 4) + '***' + text.slice(-4)} </a>;
                }
                return text;
            },
        },
    ...
    const handleDelete = async (address) => {
        ...
        localStorage.setItem('Base_addresses', JSON.stringify(data.filter(item => item.address !== address)));
    }
            ...
            for (let address of addresses) {
                ...
                const promiseFunction = () => new Promise(async (resolve, reject) => {
                    try {
                        ...
                        const response = await getBaseData(address);
                        setData(prevData => {
                            const updatedData = [...prevData];
                            const index = updatedData.findIndex(item => item.address === address);
                            if (index !== -1) {
                ...
                if (index !== -1) {
                    const promiseFunction = () => new Promise(async (resolve, reject) => {
                        try {
                            ...
                            const response = await getBaseData(data[index].address);
                            setData(prevData => {
                                const updatedData = [...prevData];
                                updatedData[index] = {
                                    ...updatedData[index],
                                ...
                                localStorage.setItem('Base_addresses', JSON.stringify(updatedData));
                                return updatedData;
                            });
                            resolve();
                        } catch (error) {
